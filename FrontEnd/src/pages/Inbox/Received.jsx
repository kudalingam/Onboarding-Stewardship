    ...
    async function getReceived() {
      try {
        const response = await axios.get(
          `http://localhost:4000/inbox/received/all/${user_id}`
        );
    ...
    <>
      {Received && (
        <div className="row d-flex justify-content-evenly">
          {Received.map((value) => {
            return <MyReceived Received={value} key={value.message_id} />;
          })}
        </div>
      )}
      ...
      <div className="card col-12 my-3 ms-3 rounded">
        <div className="card-body">
          <div className="row">
            <div className="col-3">
              <p>
                <span className="text-secondary fw-bold">{senderName}</span> -
                <span className="text-info text-muted"> {senderType}</span>
              </p>
            </div>
            <div className="col-6">
              <p>
